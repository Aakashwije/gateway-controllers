...
- The `provider` values in `mappings` and `defaultProvider` must match the `id` of a translator (and an upstream cluster) configured on the same proxy.
