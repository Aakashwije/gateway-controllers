...
- The `provider` values in `mappings` and `defaultProvider` must match the translator's `provider-id` (and an upstream cluster) configured on the same proxy.
