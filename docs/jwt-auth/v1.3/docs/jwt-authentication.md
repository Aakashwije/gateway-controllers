...
| `scopes` | object | No | Scope requirements as `allOf` (every listed scope must be present) and/or `anyOf` (at least one must be present). When both are given they are AND-ed: `(all of allOf) AND (any of anyOf)`. Takes precedence over `requiredScopes`. See [Scope and Claim Requirements](#scope-and-claim-requirements). |
| `claims` | object | No | Claim requirements as `allOf` and/or `anyOf` lists of matchers, each `{ claim, values }`. A matcher is satisfied when the token's claim value is one of `values` (for a multi-valued claim, any overlap). `allOf` requires every matcher; `anyOf` requires at least one; both are AND-ed. Takes precedence over `requiredClaims`. |
